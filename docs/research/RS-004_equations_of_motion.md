...
**Status:** Specification. **Partially implemented:** the rotational derivative of §4.1,
$\dot{\boldsymbol{\omega}} = \mathbf{J}^{-1}[\mathbf{M} - \boldsymbol{\omega}\times(\mathbf{J}\boldsymbol{\omega})]$,
exists in `radius/dynamics/rotational.py` and is verified against V-EOM-05. The
translational equation (§3), the mass equation (§6), the assembled state derivative and
every moment model are specified and not yet implemented.
...
| $-\boldsymbol{\omega}\times(\mathbf{J}\boldsymbol{\omega})$ | gyroscopic coupling. Source of coning and of intermediate-axis instability | **Implemented** in `radius/dynamics/rotational.py`, verified against V-EOM-05 (`tests/test_dynamics_rotational.py`); the term is evaluated, not integrated. The pre-implementation audit's own integration of this term was conservative — $\lVert\mathbf{h}\rVert$ drift $5\times10^{-16}$, energy drift $2\times10^{-14}$ over 8 s (audit §5.4) — an audit calculation, not a property of RADIUS code |
